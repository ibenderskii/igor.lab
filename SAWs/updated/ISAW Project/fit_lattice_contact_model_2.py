...
def run_quick_test() -> int:
    ...
    print("Quick test 12: saturating_cooperative_contact matches its definition")
    u_sat = make_contact_u_fn(
        "saturating_cooperative_contact", Tref, Tscale, n_beads=N_QUICK
    )
    A0_t, qs_t = 3.0, 0.30

    # (a) the exact formula, transcribed independently from the definition
    exact_ok = True
    for T in (285.0, 320.0, 355.0):
        b_t = _b_hs(np.array([h_true, s_true]), T, Tref, Tscale)
        qq = m_centers / float(N_QUICK)
        want = N_QUICK * (
            b_t * qq - A0_t * qq ** 2 / (1.0 + (qq / qs_t) ** 2)
        )
        got = u_sat(np.array([h_true, s_true, A0_t, qs_t]), T, m_centers)
        exact_ok &= bool(np.allclose(got, want, rtol=0, atol=1e-9))
    check(exact_ok, "u equals N*[b(T)*q - A0*q^2/(1+(q/q_sat)^2)] with q = m/N")

    # (b) A0 = 0 reproduces the hs potential bit for bit (not merely closely)
    nested_sat = all(
        np.array_equal(
            u_sat(np.array([h_true, s_true, 0.0, qs_t]), T, m_centers),
            u_hs(np.array([h_true, s_true]), T, m_centers),
        )
        for T in (285.0, 320.0, 355.0)
    )
    check(nested_sat, "A0=0 reproduces the hs potential exactly (no drift)")

    # (c) the cooperative term saturates: the gap to b(T)*m stops growing and
    #     the marginal slope returns to b(T)
    m_hi = np.array([1.0, 10.0, 100.0, 1000.0]) * float(N_QUICK) * qs_t
    b_hi = _b_hs(np.array([h_true, s_true]), 320.0, Tref, Tscale)
    gap = b_hi * m_hi - u_sat(np.array([h_true, s_true, A0_t, qs_t]), 320.0, m_hi)
    plateau = N_QUICK * A0_t * qs_t ** 2
    dm = 1e-4
    slope_hi = (
        u_sat(np.array([h_true, s_true, A0_t, qs_t]), 320.0, m_hi[-1] + dm)
        - u_sat(np.array([h_true, s_true, A0_t, qs_t]), 320.0, m_hi[-1] - dm)
    ) / (2.0 * dm)
    check(
        bool(np.all(np.diff(gap) > 0.0))
        and abs(float(gap[-1]) - plateau) < 1e-3 * plateau
        and abs(float(slope_hi) - b_hi) < 1e-4 * abs(b_hi),
        f"cooperative term saturates at N*A0*q_sat^2={plateau:.4g} "
        f"(got {float(gap[-1]):.4g}) and du/dm returns to b(T)",
    )

    # (d) invalid parameters are refused rather than silently evaluated
    bad_ok = True
    for bad in ([h_true, s_true, -1e-9, qs_t], [h_true, s_true, A0_t, 0.0],
                [h_true, s_true, A0_t, -0.1]):
        try:
            u_sat(np.array(bad), 320.0, m_centers)
        except ValueError:
            continue
        bad_ok = False
    check(bad_ok, "A0 < 0 and q_sat <= 0 raise ValueError")
