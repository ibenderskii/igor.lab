...
def energy(chain:List[Vec], occ:set[Vec], eps:float) -> float:
    """
    Zimm–Bragg-like cooperative contact energy.

    Base attraction: each *unique* non-bonded nearest-neighbour contact contributes -eps.
    Cooperativity: pay a nucleation penalty +J per *connected cluster* of contacts.
      E = -eps * m  +  J * n_clusters

    Interpretation:
      - Creating the first contact of a new cluster costs (-eps + J)
      - Adding a contact onto an existing cluster costs (-eps)
      - Bridging/merging two clusters gains (-eps - J)

    Tune J below. Setting J close to eps makes nucleation hard and growth easy (sharper transition).
    """
    J = 1.2 * eps  # cooperativity (nucleation penalty). Try 0.0–1.2*eps.

    N = len(chain)
    pos_to_idx = {pos: i for i, pos in enumerate(chain)}

    # Build unique contact edges (i,j) with i<j
    edges = set()
    ...
    # Count connected components of the contact graph, restricted to nodes with >=1 contact
...
def main() -> None:
    ...
    ap.add_argument('--eps',      type=float, default=92.7,   help='contact energy ')
    ap.add_argument('--T',        type=float, default=1,   help='temperature (k_B=1)')
