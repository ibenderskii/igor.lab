...
def energy(chain:List[Vec], occ:set[Vec], dh:float,ds:float, T:float) -> float:
    ...
    i =0.5*i
    return i*(dh-T*ds)        # each contact counted twice
...
def main() -> None:
    ...
    ap.add_argument('--dh',      type=float, default=2,   help='contact energy ')
    ap.add_argument('--ds',      type=float, default=1,   help='contact energy ')
