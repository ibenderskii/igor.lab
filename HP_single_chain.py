...
# ---------- energy (type-dependent, T-dependent eps) ----------
def energy(chain:List[Vec], occ:set, types:List[str], T:float, eps_fn) -> float:
    """
    Return total energy: negative for attractions
    energy subtracts eps_fn(type_i,type_j,T) for each non-bonded nearest-neighbour contact
    """
...
def main() -> None:
    ...
    # sigmoid params (defaults pick transition near T=2.0)
    ap.add_argument('--eps_HH_low',  type=float, default=0.05)
    ap.add_argument('--eps_HH_high', type=float, default=3.5)
    ...
    print(f"eps_HH(Tmid={args.eps_Tmid}) = {eps_fn('H','H', args.eps_Tmid):.3f}")
    ...
    for step in range(1, args.steps+1):
        ...
        if not ok:
            continue
        dE = energy(chain_new, occ_new, types, args.T, eps_fn) - E
